...
export default class MessageController {
    ...
    public async getMessages( req: Request ): Promise<ListResponse<Message>> {
        ...
        return await this.messageService.getMessages( req.params.conversationId, { page, limit }, req.auth )
    }
    ...
    public async sendMessage( req: Request, res: Response ): Promise<Response<Message>> {
        ...
        const message = await this.messageService.sendMessage( conversationId, {
            body,
            image,
            type
        }, req.auth )
        ...
        this.conversationService.getUnreadConversationsCount( message.recipient.id ).then( ( count ) => {
            if ( count > 0 ) {
    ...
    public async seenMessages( req: Request ): Promise<Message[]> {
        ...
        if ( lastMessage ) {
            ...
            SocketService.emit( `conversation.unread.count.${ user.id }`, await this.conversationService.getUnreadConversationsCount( user.id ) )
            SocketService.emit( `message.seen`, lastMessage )
        }
        return messages
    }
    ...
    public async sendReaction( req: Request ): Promise<message> {
        ...
        const message = await this.messageService.sendReaction( { conversationId, messageId, name }, req.auth )
